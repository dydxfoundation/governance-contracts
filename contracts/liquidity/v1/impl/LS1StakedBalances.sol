 ...
 */
abstract contract LS1StakedBalances is LS1Rewards {
   ...
   */
  function getActiveBalanceCurrentEpoch(address staker) public view returns (uint256) {
   ...
   */
  function getActiveBalanceNextEpoch(address staker) public view returns (uint256) {
    if (!hasEpochZeroStarted()) {
      return 0;
    }
    (LS1Types.StoredBalance memory balance, , , ) = _loadActiveBalance(_ACTIVE_BALANCES_[staker]);
   ...
   */
  function getInactiveBalanceCurrentEpoch(address staker) public view returns (uint256) {
    if (!hasEpochZeroStarted()) {
      return 0;
    }
   ...
   */
  function getStakerDebtBalance(address staker) public view returns (uint256) {
    if (!hasEpochZeroStarted()) {
      return 0;
    }
  ...
  function _increaseCurrentAndNextActiveBalance(address staker, uint256 amount) internal {
  ...
  function _moveNextBalanceActiveToInactive(address staker, uint256 amount) internal {
  ...
  function _decreaseCurrentAndNextInactiveBalance(address staker, uint256 amount) internal {
  ...
  function _settleStakerDebtBalance(address staker) internal returns (uint256) {
  ...
  function _settleAndClaimRewards(address staker, address recipient) internal returns (uint256) {
    // Always settle total active balance before settling a staker active balance.
    uint256 totalBalance = _settleTotalActiveBalance();
  ...
  function _applyShortfall(uint256 shortfallAmount, uint256 shortfallIndex) internal {
   ...
   */
  function _failsafeSettleUserInactiveBalance(address staker, uint256 maxEpoch) internal {
   ...
   */
  function _failsafeDeleteUserInactiveBalance(address staker) internal {
    LS1Types.StoredBalance storage balancePtr = _getBalancePtr(staker, false);
    LS1Types.StoredBalance memory balance =
   ...
   */
  function _settleBalance(address maybeStaker, bool isActiveBalance) private returns (uint256) {
  ...
  function _loadBalanceForUpdate(
    ...
    address maybeStaker,
    bool isActiveBalance
  ) private returns (LS1Types.StoredBalance memory) {
    // Active balance.
    if (isActiveBalance) {
      (
        LS1Types.StoredBalance memory balance,
  ...
  function _loadActiveBalance(LS1Types.StoredBalance storage balancePtr)
    private
    view
    returns (
  ...
  function _loadTotalInactiveBalance(LS1Types.StoredBalance storage balancePtr)
    private
    view
  ...
  function _loadUserInactiveBalance(LS1Types.StoredBalance storage balancePtr)
    private
    view
  ...
  function _applyShortfallsToBalance(LS1Types.StoredBalance memory balance)
    private
    view
    returns (LS1Types.StoredBalance memory, uint256)
  {
