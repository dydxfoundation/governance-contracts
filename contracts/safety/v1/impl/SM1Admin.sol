...
abstract contract SM1Admin is SM1StakedBalances {
    ...
    nonReentrant
  {
    if (!hasEpochZeroStarted()) {
      require(block.timestamp < offset, 'SM1Admin: Cannot jump past start of epoch zero');
    ...
    require(originalCurrentEpoch == newCurrentEpoch, 'SM1Admin: Cannot jump between epochs');
  }
