...
hardhatTask('deploy:v3-data-availability-proposal', 'Create proposal to fix data availability bug on v3 perp contract.')
  .addParam('proposalIpfsHashHex', 'IPFS hash for the uploaded DIP describing the proposal', DIP_22_IPFS_HASH, types.string)
  .addParam('governorAddress', 'Address of the deployed DydxGovernor contract', mainnetAddresses.governor, types.string)
  .addParam('starkwarePriorityAddress', 'Address of the deployed Starkware Priority Executor contract', mainnetAddresses.starkwarePriorityTimelock, types.string)
  .addParam('starkPerpetualAddress', 'Address of the deployed StarkPerpetual contract', mainnetAddresses.starkPerpetual, types.string)
