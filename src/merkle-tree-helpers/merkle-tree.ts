// Based on https://github.com/Uniswap/merkle-distributor/blob/c3255bfa2b684594ecd562cacd7664b0f18330bf/src/merkle-tree.ts
...
export default class MerkleTree {
  ...
  constructor(elements: Buffer[]) {
    ...
    // Create layers
    this.layers = this._getLayers(this.elements);
  }

  protected _getLayers(elements: Buffer[]): Buffer[][] {
  ...
  protected _getNextLayer(elements: Buffer[]): Buffer[] {
  ...
  protected _getProof(el: Buffer): Buffer[] {
  ...
  protected _getHexProof(el: Buffer): string[] {
    const proof = this._getProof(el);
