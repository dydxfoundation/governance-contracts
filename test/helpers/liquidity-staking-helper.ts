...
import {
  ...
  ZERO_ADDRESS,
  OWNER_ROLE_KEY,
  EPOCH_PARAMETERS_ROLE_KEY,
  REWARDS_RATE_ROLE_KEY,
  BORROWER_ADMIN_ROLE_KEY,
  CLAIM_OPERATOR_ROLE_KEY,
  STAKE_OPERATOR_ROLE_KEY,
  DEBT_OPERATOR_ROLE_KEY,
} from '../../helpers/constants';
import { increaseTimeAndMine, timeLatest } from '../../helpers/misc-utils';
...
import { StoredBalance } from './stored-balance';
...
const SNAPSHOTS: Record<string, LiquidityStakingState> = {};
...
export interface LiquidityStakingState {
...
function customCloner(value: any) {
  if (typeof value.clone === 'function') {
    return value.clone();
  }
}

export class LiquidityStakingHelper {
  private contract: LiquidityStakingV1;
  private token: MockStakedToken;
  private vault: SignerWithAddress;
  private admin: LiquidityStakingV1;
  private users: Record<string, LiquidityStakingV1>;
  private signers: Record<string, Signer>;
  private roles: Record<string, string>;

  // Contract state.
  state: LiquidityStakingState;

  constructor(
    liquidityStakingV1: LiquidityStakingV1,
    token: MockStakedToken,
    vault: SignerWithAddress,
    admin: SignerWithAddress,
    users: SignerWithAddress[]
  ) {
    this.contract = liquidityStakingV1;
    this.token = token;
    this.vault = vault;
    this.admin = liquidityStakingV1.connect(admin.signer);
    this.users = {};
    this.signers = {};
    this.roles = {};
    for (const signer of users) {
      this.users[signer.address] = liquidityStakingV1.connect(signer.signer);
      this.signers[signer.address] = signer.signer;
    }
  ...
  async setBorrowerAllocations(allocations: Record<string, number>): Promise<void> {
    ...
    await this.admin.setBorrowerAllocations(addresses, points);
    ...
    for (let i = 0; i < addresses.length; i++) {
      const allocationNext = await this.contract.getAllocationFractionNextEpoch(addresses[i]);
    ...
    if (!(await this.contract.hasEpochZeroStarted())) {
      for (let i = 0; i < addresses.length; i++) {
        const allocationCurrent = await this.contract.getAllocationFractionCurrentEpoch(
          addresses[i]
        );
    ...
    const curZero = await this.contract.getAllocationFractionCurrentEpoch(ZERO_ADDRESS);
    const curSum = curZero.add(
      await this.sumByAddr((addr) => this.contract.getAllocationFractionCurrentEpoch(addr))
    );
    expectEq(curSum, BORROWING_TOTAL_ALLOCATION, 'setBorrowerAllocations: curSum');
    const nextZero = await this.contract.getAllocationFractionNextEpoch(ZERO_ADDRESS);
    const nextSum = nextZero.add(
      await this.sumByAddr((addr) => this.contract.getAllocationFractionNextEpoch(addr))
    );
  ...
  async setBorrowingRestriction(
    ...
    const tx = await this.contract.setBorrowingRestriction(borrowerAddress, isRestricted);

    // Get previous status.
    const wasRestricted = await this.contract.isBorrowingRestrictedForBorrower(borrowerAddress);
    ...
    // Check new status.
    expect(await this.contract.isBorrowingRestrictedForBorrower(borrowerAddress)).to.be.equal(
      isRestricted,
      'setBorrowingRestriction'
    );
  }

  async addOperator(operator: string | SignerWithAddress, roleKey: string): Promise<void> {
    const address = asAddress(operator);
    const allRoles: Record<string, string> = await this.getRoles();
    const role: string = allRoles[roleKey];
    await expect(this.contract.grantRole(role, address))
      .to.emit(this.contract, 'RoleGranted')
      .withArgs(role, address, await this.contract.signer.getAddress());

    expect(await this.contract.hasRole(role, address)).to.be.true;
  }

  async removeOperator(operator: string | SignerWithAddress, roleKey: string): Promise<void> {
    const address = asAddress(operator);
    const allRoles: Record<string, string> = await this.getRoles();
    const role: string = allRoles[roleKey];
    await expect(this.contract.revokeRole(role, address))
      .to.emit(this.contract, 'RoleRevoked')
      .withArgs(role, address, await this.contract.signer.getAddress());

    expect(await this.contract.hasRole(role, address)).to.be.false;
  }
  ...
  async stake(
    ...
    await this.state.activeBalanceByStaker[address].increaseCurrentAndNext(amount);
  ...
  async withdrawDebt(
    ...
    await expect(signer.withdrawDebt(recipientAddress, amount))
    ...
    // Check getters.
    expectEq(
      await this.contract.getStakerDebtBalance(address),
  ...
  async withdrawMaxDebt(
    ...
    const amount = await this.contract.getDebtAvailableToWithdraw(address);
    ...
    await expect(signer.withdrawMaxDebt(recipientAddress))
      .to.emit(this.contract, 'WithdrewDebt')
      .withArgs(address, recipientAddress, amount, newDebtBalance);
  ...
  async claimRewards(
    ...
    const end = BigNumber.from(endTimestamp || (await timeLatest()).toNumber());
    const expectedRewards = end.sub(startTimestamp).mul(rewardsRate).mul(stakerShare);
    ...
    expectEqualExceptRounding(
      ...
      {
        ...options,
        roundingTolerance: rewardsRate.mul(2).toNumber(),
      },
  ...
  async borrowViaProxy(
    ...
    const address = asAddress(borrower);
  ...
  async borrow(
    ...
    const signer = this.users[address];
    const sendTx = async (newBorrowedBalance: BigNumberish) => {
      await expect(signer.borrow(amount))
        .to.emit(this.contract, 'Borrowed')
        .withArgs(address, amount, newBorrowedBalance);
    };
    return this._borrow(sendTx, address, amount, options);
  ...
  async _borrow(
    ...
    // Check getters.
    expectEq(
      await this.contract.getBorrowedBalance(address),
  ...
  async repayLoanViaProxy(
    ...
    const sendTx = async (newBorrowerBalance: BigNumberish) => {
      await expect(borrower.repayLoan(amount))
        .to.emit(this.contract, 'RepaidLoan')
        .withArgs(address, address, amount, newBorrowerBalance);
    };
    return this._repayLoan(sendTx, address, borrower, amount, options);
  }
  ...
  async repayLoan(
    ...
    const signer = this.users[address];
    const sendTx = async (newBorrowerBalance: BigNumberish) => {
      await expect(signer.repayLoan(borrowerAddress, amount))
        .to.emit(this.contract, 'RepaidLoan')
  ...
  async _repayLoan(
    ...
    expectEq(
      contractBalanceAfter.sub(contractBalanceBefore),
      amount,
      'repayLoan: Increase contract balance'
    );
    expectEq(
      borrowerBalanceBefore.sub(borrowerBalanceAfter),
      amount,
      'repayLoan: Decrease borrower balance'
    );
  ...
  async repayDebt(
    ...
    await expect(signer.repayDebt(borrowerAddress, amount))
    ...
    // Check getters.
    expectEq(
      await this.contract.getTotalDebtAvailableToWithdraw(),
  ...
  async autoPay(
    ...
    const [borrowed, loan, debt] = ((await borrower.callStatic.autoPayOrBorrow()) as unknown) as [
      BigNumber,
      BigNumber,
      BigNumber
    ];
    const tx = await borrower.autoPayOrBorrow();
    const borrowedBalance = await this.contract.getBorrowedBalance(borrower.address);
    const debtBalance = await this.contract.getBorrowerDebtBalance(borrower.address);
    if (!borrowed.eq(0)) {
      await expect(tx).to.emit(borrower, 'Borrowed').withArgs(borrowed, borrowedBalance);
    }
    if (!loan.eq(0)) {
      await expect(tx).to.emit(borrower, 'RepaidLoan').withArgs(loan, borrowedBalance, false);
    }
    if (!debt.eq(0)) {
    ...
    // Update state.
    const address = asAddress(borrower);
    const borrowDelta = borrowed.sub(loan);
    ...
    expectEq(loanAfter, 0, 'autoPay: loanAfter');
    expectEq(debtAfter, 0, 'autoPay: debtAfter');

    // Check invariants.
    await this.checkInvariants(options);

    return [borrowed, loan, debt];
  }
  ...
  async markDebt(
    ...
    const tx = await this.contract.markDebt(borrowers);
    ...
    for (const restrictedLog of restricted) {
      ...
      // Check getters.
      expect(await this.contract.isBorrowingRestrictedForBorrower(restrictedBorrower)).to.be.true;
    }

    // Update state and check invariants.
    this.state.netDeposits = this.state.netDeposits.sub(newDebtSum);
    await Promise.all(
      _.map(borrowersWithExpectedDebts, async (newDebt, borrower) => {
  ...
  async expectNoShortfall(): Promise<void> {
    await expect(this.contract.markDebt([])).to.be.revertedWith('No shortfall has occurred');
  }
  ...
  async decreaseStakerDebt(
    ...
    await expect(signer.decreaseStakerDebt(stakerAddress, amount))
    ...
    // Check getters.
    expectEq(
      await this.contract.getStakerDebtBalance(stakerAddress),
  ...
  async decreaseBorrowerDebt(
    ...
    await expect(signer.decreaseBorrowerDebt(borrowerAddress, amount))
    ...
    // Check getters.
    expectEq(
      await this.contract.getBorrowerDebtBalance(borrowerAddress),
  ...
  async fullBorrowViaProxy(
    ...
    expect(await this.contract.getBorrowableAmount(borrower.address)).to.equal(amount);
  ...
  async fullRepayDebt(
    ...
    expect(await this.contract.getBorrowerDebtBalance(borrower.address)).to.equal(amount);
    await this.repayDebt(borrower, borrower, amount, options);
    await expect(this.repayDebt(borrower, borrower, 1)).to.be.revertedWith(
      'LS1Borrowing: Repay debt amount exceeds borrower debt'
    );
  }
  ...
  async fullWithdrawDebt(
    ...
    const contractAvailable = await this.contract.getTotalDebtAvailableToWithdraw();
    const stakerAvailable = await this.contract.getStakerDebtBalance(staker.address);
    ...
    expectEq(
      await this.contract.getDebtAvailableToWithdraw(staker.address),
      available,
  ...
  async checkInvariants(options: InvariantCheckOptions = {}): Promise<void> {
    ...
    // Staked balance accounting; let X = Deposits - Loan Withdrawals - Debt Conversions; then...
    //
    // X = Total Borrowed + Contract Balance – Debt Available to Withdraw – Naked ERC20 Transfers In
    const borrowed = await this.contract.getTotalBorrowedBalance();
    const balance = await this.token.balanceOf(this.contract.address);
    const availableDebt = await this.contract.getTotalDebtAvailableToWithdraw();
    expectEq(
    ...
    const totalBorrowerDebt = await this.contract.getTotalBorrowerDebtBalance();
    const borrowerDebt = await this.sumByAddr((addr) => this.contract.getBorrowerDebtBalance(addr));
    ...
    if (!options.skipStakerVsBorrowerDebtComparison) {
      const stakerDebt = await this.sumByAddr((addr) => {
        return this.contract.getStakerDebtBalance(addr);
      });
    ...
    const availableToWithdraw = await this.contract.getTotalDebtAvailableToWithdraw();
  ...
  async elapseEpochWithExpectedBalanceUpdates(
    ...
    await Promise.all(
      _.map(checkInactiveBalanceUpdates, async (values, address) => {
        expectEq(await this.contract.getInactiveBalanceCurrentEpoch(address), values[1]),
          `elapseEpoch(${currentEpoch} -> ${
            currentEpoch + 1
  ...
  async expectBorrowerDebt(expectedDebtByBorrower: Record<string, BigNumberish>): Promise<void> {
    await Promise.all(
      _.map(expectedDebtByBorrower, async (value, address) => {
        expectEq(
          await this.contract.getBorrowerDebtBalance(address),
          value,
  ...
  async elapseEpoch(): Promise<void> {
    ...
    if (remaining.eq(0)) {
      await increaseTimeAndMine(EPOCH_LENGTH.toNumber());
    } else {
  ...
  // singleton for fetching all roles in the contract. Fetches roles if
  // this.roles is empty, then returns this.roles.
  async getRoles(): Promise<Record<string, string>> {
    if (_.isEmpty(this.roles)) {
      const roles: Record<string, string> = {};
      roles[OWNER_ROLE_KEY] = await this.contract.OWNER_ROLE();
      roles[EPOCH_PARAMETERS_ROLE_KEY] = await this.contract.EPOCH_PARAMETERS_ROLE();
      roles[REWARDS_RATE_ROLE_KEY] = await this.contract.REWARDS_RATE_ROLE();
      roles[BORROWER_ADMIN_ROLE_KEY] = await this.contract.BORROWER_ADMIN_ROLE();
      roles[CLAIM_OPERATOR_ROLE_KEY] = await this.contract.CLAIM_OPERATOR_ROLE();
      roles[STAKE_OPERATOR_ROLE_KEY] = await this.contract.STAKE_OPERATOR_ROLE();
      roles[DEBT_OPERATOR_ROLE_KEY] = await this.contract.DEBT_OPERATOR_ROLE();
  ...
  private makeInitialState(): LiquidityStakingState {
    return {
  ...
  private async parseLogs(
    ...
    return result.logs.map((log) => this.contract.interface.parseLog(log));
  }
...
function expectEqualExceptRounding(
  ...
  expect(BigNumber.from(a).toNumber()).to.be.approximately(
    BigNumber.from(b).toNumber(),
    tolerance,
    `${message || ''}: ${a} ≠ ${b} (tolerance: ${tolerance})`
  );
}
