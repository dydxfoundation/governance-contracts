...
makeSuite('LS1ERC20', (testEnv: TestEnv) => {
  // Contracts.
  ...
  describe('symbol', () => {
    it('Has correct symbol', async () => {
      expect(await liquidityStakingV1.symbol()).to.equal('dydxUSDC');
    });
  });
  ...
  describe('transfer', () => {
    ...
    it('User cannot transfer funds if they have not staked', async () => {
      ...
      ).to.be.revertedWith('LS1ERC20: Transfer amount exceeds user next epoch active balance');
    });
