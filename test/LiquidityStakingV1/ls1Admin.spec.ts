...
import { SignerWithAddress, makeSuite, TestEnv } from '../helpers/make-suite';
...
makeSuite('LS1Admin', (testEnv: TestEnv) => {
  ...
  before(async () => {
    ...
    // The offset is the start of epoch zero.
    // Default offset (deploy.ts) is 2.14 epochs in the future.
    initialOffset = distributionStart;
  ...
  describe('Before epoch zero has started', () => {
    ...
    it("Can set valid epoch & blackout parameters which don't jump past the start of epoch zero", async () => {
      ...
      ).to.be.revertedWith('LS1EpochSchedule: Epoch length must be at most the maximum');
    ...
    it('Cannot set epoch parameters which jump past the start of epoch zero', async () => {
      ...
      await expect(contract.setEpochParameters(EPOCH_LENGTH, currentTime)).to.be.revertedWith(
        'LS1Admin: Cannot jump past start of epoch zero'
      );
      await expect(
        contract.setEpochParameters(EPOCH_LENGTH, currentTime - 10000)
      ).to.be.revertedWith('LS1Admin: Cannot jump past start of epoch zero');
    });
    ...
    it('Cannot set allocations which sum to greater than one', async () => {
      ...
      ).to.be.revertedWith('LS1BorrowerAllocations: Total allocated borrow units changed');
    });
    ...
    it('Cannot call with invalid params', async () => {
      await expect(liquidityStakingV1.setBorrowerAllocations([], [1])).to.be.revertedWith(
        'LS1Admin: Array params length mismatch'
      );
    });
  });
  ...
  describe('After epoch zero has started', () => {
    before(async () => {
      ...
      // Move us to the middle of epoch 2.
      const newTimestamp = initialOffset.toNumber() + EPOCH_LENGTH.toNumber() * 2.5;
    ...
    it('Can set epoch parameters which maintain current epoch and blackout status', async () => {
      ...
      //                     |   |   |   |
      //                     0   1   2   3
      await contract.setBlackoutWindow(BLACKOUT_WINDOW.div(2)); // Can be at most half epoch length.
    ...
    it('Cannot set epoch parameters which move us into the blackout window', async () => {
      ...
      ).to.be.revertedWith('LS1Admin: Cannot jump into blackout window');
    });
    ...
    it('Cannot set blackout parameters which move us into the blackout window', async () => {
      ...
      await expect(contract.setBlackoutWindow(EPOCH_LENGTH.div(2))).to.be.revertedWith(
        'LS1Admin: Cannot jump into blackout window'
      );
    });
    ...
    it('Cannot set epoch parameters which decrease the current epoch number', async () => {
      ...
      ).to.be.revertedWith('LS1Admin: Cannot jump between epochs');
    });
  ...
  describe('after borrower allocations are set', async () => {
    ...
    it('can set and remove borrowing restrictions', async () => {
      ...
      await expect(contract.borrow(borrowers[1], 1)).to.be.revertedWith(
        'LS1Borrowing: Borrowing is restricted'
      );
      ...
      await contract.repayLoan(borrowers[0], borrowers[0], stakerInitialBalance * 0.2);
