...
makeSuite('LS1DebtAccounting', (testEnv: TestEnv) => {
  ...
  describe('Simple shortfall scenarios', () => {
    ...
    it('Allows restricting a borrower even if there is no shortfall', async () => {
      ...
      // Borrower 0 repays their loan.
      await contract.repayLoan(borrowers[0], borrowers[0], 40e10);
      ...
      // Borrower who did not repay their loan can be restricted.
    ...
    it('Cannot mark debt if provided borrowers do not cover the shortfall', async () => {
      ...
      ).to.be.revertedWith('LS1DebtAccounting: The borrowers did not cover the shortfall');
      await expect(
  ...
  describe('Debt accounting, after two shortfalls', () => {
    before(async () => {
      ...
      await contract.repayLoan(borrowers[1], borrowers[1], 200);
    ...
    it('getters return shortfall info', async () => {
      expect(await liquidityStakingV1.getShortfallCounter()).to.equal(2);
      expect(await liquidityStakingV1.getShortfallEpoch(0)).to.equal(1);
      expect(await liquidityStakingV1.getShortfallEpoch(1)).to.equal(2);
      expect(await liquidityStakingV1.getShortfallIndex(0)).to.equal(
        SHORTFALL_INDEX_BASE.mul(5).div(10)
      );
      expect(await liquidityStakingV1.getShortfallIndex(1)).to.equal(
    ...
    it('repaid debt cannot be withdrawn as a loan', async () => {
      // Check initial state.
      //
