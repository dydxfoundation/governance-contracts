...
makeSuite('IncentivesController handleAction tests', (testEnv) => {
  for (const {
    ...
    it(caseName, async () => {
      ...
      const rewardsBalanceBefore = await incentivesController.getUserUnclaimedRewards(userAddress);
      ...
      const handleActionReceipt = await waitForTx(
        await aDaiMock.handleActionOnIc(userAddress, userBalance, totalSupply)
      );
      ...
      const userIndexAfter = await getUserIndex(incentivesController, userAddress, underlyingAsset);
      ...
      const rewardsBalanceAfter = await incentivesController.getUserUnclaimedRewards(userAddress);
