...
export class StakingHelper {
  ...
  constructor(
    ...
    this.admin = contract.connect(admin.signer);
    this.users = {};
    this.signers = {};
    this.roles = {};
    for (const signer of users) {
      this.users[signer.address] = contract.connect(signer.signer);
  ...
  async mintAndApprove(account: string | SignerWithAddress, amount: BigNumberish): Promise<void> {
    ...
    await this.token.connect(this.tokenSource.signer).transfer(address, amount);
  ...
  async stake(
    ...
    if (this.isSafetyModule) {
      ...
      const exchangeRateBase = await this.contract.EXCHANGE_RATE_BASE();
  ...
  async repayBorrow(
    ...
    const signer = this.users[address];
    const sendTx = async (newBorrowerBalance: BigNumberish) => {
      await expect(signer.repayBorrow(borrowerAddress, amount))
        .to.emit(this.contract, 'RepaidBorrow')
  ...
  async expectNoShortfall(): Promise<void> {
    await expect(this.contract.markDebt([])).to.be.revertedWith('LS1DebtAccounting: No shortfall');
  }
