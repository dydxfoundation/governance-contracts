...
makeSuite('StakedDydxToken. Transfers', (testEnv: TestEnv) => {
  ...
  it('User 1 transfers 50 SDYDX to User 5', async () => {
    ...
    await compareRewardsAtTransfer(stakedDydxToken, sender, receiver, amount, true, false);
  });

  it('User 5 transfers 50 SDYDX to himself', async () => {
    const { stakedDydxToken, users } = testEnv;
    const amount = ethers.utils.parseEther('50');
    const sender = users[5];
    await compareRewardsAtTransfer(stakedDydxToken, sender, sender, amount, true, true);
  });

  it('User 5 transfers 50 SDYDX to user 2, with rewards not enabled', async () => {
    const { stakedDydxToken, dydxToken, users } = testEnv;
    const amount = ethers.utils.parseEther('50');
    const sender = users[5];
  ...
  it('User 4 stakes and transfers 50 SDYDX to user 2, with rewards not enabled', async () => {
    const { stakedDydxToken, dydxToken, users } = testEnv;
    const amount = ethers.utils.parseEther('50');
    ...
    const actions = () => [
      dydxToken.connect(sender.signer).approve(stakedDydxToken.address, amount),
      stakedDydxToken.connect(sender.signer).stake(sender.address, amount),
    ];

    await compareRewardsAtAction(stakedDydxToken, sender.address, actions, false, assetConfig);
    await compareRewardsAtTransfer(
      stakedDydxToken,
      sender,
      receiver,
  ...
  it('Activate cooldown of User2, transfer entire amount from User2 to User3, cooldown of User2 should be reset', async () => {
    ...
    const amount = await stakedDydxToken.balanceOf(sender.address);
    ...
    await stakedDydxToken.connect(sender.signer).cooldown();
    const cooldownActivationTimestamp = await (await timeLatest()).toString();

    const cooldownTimestamp = await stakedDydxToken.stakersCooldowns(sender.address);
    ...
    const cooldownTimestampAfterTransfer = await (
      await stakedDydxToken.stakersCooldowns(sender.address)
    ).toString();
  ...
  it('Transfer balance from User 3 to user 2 cooldown  of User 2 should be reset if User3 cooldown expired', async () => {
    ...
    await dydxToken.connect(receiver.signer).approve(stakedDydxToken.address, amount);
    await stakedDydxToken.connect(receiver.signer).stake(receiver.address, amount);
    await stakedDydxToken.connect(receiver.signer).cooldown();
    const receiverCooldown = await stakedDydxToken.stakersCooldowns(sender.address);
    ...
    // Transfer staked DYDX from sender to receiver, it will also transfer the cooldown status from sender to the receiver
    await compareRewardsAtTransfer(
      stakedDydxToken,
      sender,
      receiver,
    ...
    const stakerCooldownTimestampBefore = await stakedDydxToken.stakersCooldowns(receiver.address);
  ...
  it('Transfer balance from User 3 to user 2, cooldown of User 2 should be the same if User3 cooldown is less than User2 cooldown', async () => {
    ...
    const receiverCooldown = await (
      await stakedDydxToken.stakersCooldowns(receiver.address)
    ).toString();
