...
makeSuite('StakedDydxToken. Permit', (testEnv: TestEnv) => {
  it('Reverts submitting a permit with 0 expiration', async () => {
    const { deployer, users, stakedDydxToken } = testEnv;
    ...
    const expiration = 0;
    const nonce = (await stakedDydxToken._nonces(owner)).toNumber();
    ...
    if (!ownerPrivateKey || (await new Wallet(ownerPrivateKey).getAddress()) !== owner) {
      throw new Error('INVALID_OWNER_PK');
    }

    expect((await stakedDydxToken.allowance(owner, spender)).toString()).to.be.equal(
      '0',
  ...
  it('Submits a permit with maximum expiration length', async () => {
    ...
    expect((await stakedDydxToken._nonces(owner)).toNumber()).to.be.equal(1);
  });
  ...
  it('Cancels the previous permit', async () => {
    ...
    expect((await stakedDydxToken._nonces(owner)).toNumber()).to.be.equal(2);
  });
