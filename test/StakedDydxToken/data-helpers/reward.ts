...
export const compareRewardsAtAction = async (
  ...
  const rewardsBalanceBefore = BigNumber.from(
    await (await stakedDydxToken.getTotalRewardsBalance(userAddress)).toString()
  );
  ...
  const assetConfiguration = assetConfig
    ...
    : {
        ...
        totalStaked: await stakedDydxToken.totalSupply(),
        underlyingAsset,
      };
  await stakedDydxToken.configureAssets([assetConfiguration]);
...
export const compareRewardsAtTransfer = async (
  ...
  const underlyingAsset = stakedDydxToken.address;
  const fromSavedBalance = await stakedDydxToken.balanceOf(fromAddress);
  const toSavedBalance = await stakedDydxToken.balanceOf(toAddress);
  const fromSavedRewards = BigNumber.from(
    await (await stakedDydxToken.getTotalRewardsBalance(fromAddress)).toString()
  );
  const toSavedRewards = BigNumber.from(
    await (await stakedDydxToken.getTotalRewardsBalance(toAddress)).toString()
  );
  // Get index before actions
  const fromIndexBefore = await getUserIndex(stakedDydxToken, fromAddress, underlyingAsset);
  const toIndexBefore = await getUserIndex(stakedDydxToken, toAddress, underlyingAsset);

  // Load actions that can or not update the user index
  const actions = () => [stakedDydxToken.connect(from.signer).transfer(toAddress, amount)];
  ...
  // Get index after actions
  const fromIndexAfter = await getUserIndex(stakedDydxToken, fromAddress, underlyingAsset);
  const toIndexAfter = await getUserIndex(stakedDydxToken, toAddress, underlyingAsset);
  ...
  if (fromAddress === toAddress) {
    ...
    const fromNewBalance = await (await stakedDydxToken.balanceOf(fromAddress)).toString();
    const toNewBalance = await (await stakedDydxToken.balanceOf(toAddress)).toString();
