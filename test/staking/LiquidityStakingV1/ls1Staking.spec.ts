...
makeSuite('LS1Staking', deployPhase2, (testEnv: TestEnv) => {
  // Contracts.
  ...
  describe('withdrawStake', () => {
    ...
    it('Staker cannot withdraw funds if none are staked', async () => {
      ...
      ).to.be.revertedWith('LS1Staking: Withdraw exceeds amount available in the contract');
    });
  });
  ...
  describe('claimRewards', () => {
    ...
    it('User with staked balance can claim rewards', async () => {
      ...
      const initialRewardsRate = 0;
      await expect(liquidityStakingV1.connect(deployer.signer).setRewardsPerSecond(initialRewardsRate))
        .to.emit(liquidityStakingV1, 'RewardsPerSecondUpdated')
        .withArgs(initialRewardsRate);
    ...
    it('User with nonzero staked balance for one epoch but emission rate was zero cannot claim rewards', async () => {
      ...
      const emissionRate1 = 0;
      await expect(liquidityStakingV1.connect(deployer.signer).setRewardsPerSecond(emissionRate1))
        .to.emit(liquidityStakingV1, 'RewardsPerSecondUpdated')
        .withArgs(emissionRate1);
      ...
      const emissionRate2 = 1;
      await expect(liquidityStakingV1.connect(deployer.signer).setRewardsPerSecond(emissionRate2))
        .to.emit(liquidityStakingV1, 'RewardsPerSecondUpdated')
        .withArgs(emissionRate2);
