...
export async function executeV3DataAvailabilityNoProposal({
  ...
  await waitForTx(await (starkPerpetual as any).addImplementation(deployConfig.IMPLEMENTATION_ADDRESS, deployConfig.BYTES_IMPLEMENTATION, false));
  await waitForTx(await (starkPerpetual as any).upgradeTo(deployConfig.IMPLEMENTATION_ADDRESS, deployConfig.BYTES_IMPLEMENTATION, false));
